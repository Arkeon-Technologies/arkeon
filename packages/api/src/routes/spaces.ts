 ...
 */
async function requireSpaceRole(
...
spacesRouter.openapi(createSpaceRoute, async (c) => {
  ...
  const space = (rows as SpaceRecord[])[0];
  if (!space) {
...
spacesRouter.openapi(listSpaceEntitiesRoute, async (c) => {
  ...
  const entities = (rows as Array<Record<string, unknown>>).slice(0, limit);
  const next = (rows as Array<Record<string, unknown>>).length > limit ? entities[entities.length - 1] : null;
...
spacesRouter.openapi(addSpaceEntityRoute, async (c) => {
  ...
  const results = await sql.transaction([
    ...
    sql.query(
      ...
      [spaceId, body.entity_id, actor.id, now],
    ),
  ]);
