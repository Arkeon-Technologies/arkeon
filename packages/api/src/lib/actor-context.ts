 ...
 *
 * Sets: app.actor_id, app.actor_arke_id, app.actor_read_level, app.actor_write_level, app.actor_is_admin
 */
export function setActorContext(sql: ReturnType<typeof createSql>, actor: Actor | null) {
  if (!actor) {
    return [
      sql`SELECT set_config('app.actor_id', '', true)`,
      sql`SELECT set_config('app.actor_arke_id', '', true)`,
  ...
  return [
    ...
    sql`SELECT set_config('app.actor_arke_id', ${actor.arkeId ?? ''}, true)`,
